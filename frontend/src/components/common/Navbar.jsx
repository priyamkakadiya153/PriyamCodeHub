...
import priyamCodeHubLogo from "../../assets/Logo/Logo-Full-Dark.png";
...
const Navbar = () => {
    ...
    const getCategorySlug = (name = "") => name.trim().toLowerCase().split(" ").filter(Boolean).join("-")
    ...
    return (
        <nav className={`z-[10] flex h-14 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-white translate-y-0 transition-all ${showNavbar} `}>
            ...
            <div className='flex w-11/12 max-w-maxContent items-center justify-between '>
                {/* logo */}
                <Link to="/">
                    <img src={priyamCodeHubLogo} width={160} className='object-contain' loading='lazy' />
                </Link>
                            ...
                            <li key={index}>
                                {
                                    link.title === "Catalog" ? (
                                        <div
                                            className={`group relative flex cursor-pointer items-center gap-1 ${(matchRoute("/catalog/:catalogName") || matchRoute("/catalog/:catalogName/:categoryId"))
                                                    ...
                                                    : subLinks.length ? (
                                                        <>
                                                            {subLinks?.map((subLink, i) => (
                                                                <Link
                                                                    to={`/catalog/${getCategorySlug(subLink.name)}/${subLink._id}`}
