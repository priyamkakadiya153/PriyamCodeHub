...
const isDemoPaymentEnabled = () => {
    return process.env.DEMO_PAYMENT_MODE === "true" ||
        process.env.RAZORPAY_KEY === "replace_razorpay_key" ||
        process.env.RAZORPAY_SECRET === "replace_razorpay_secret";
}
...
exports.verifyPayment = async (req, res) => {
    ...
    if (isDemoPaymentEnabled() && isDemo === true) {
        await enrollStudents(courses, userId, res);
        return res.status(200).json({ success: true, message: "Demo payment verified and course enrolled" });
    }

    if (!razorpay_signature) {
        return res.status(400).json({ success: false, message: "Payment Failed, signature not found" });
    }
